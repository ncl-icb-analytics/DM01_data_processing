...
# Add a row with system totals
system_total <- pivot_table_with_totals %>%
  summarise(across(-Speciality, sum, .names = "{.col}")) %>% # Summing across all columns except Speciality
  mutate(Speciality = "System Total") # Add System Total label

# Combine the total row with the original pivot table
final_table <- pivot_table_with_totals %>%
  bind_rows(system_total) %>%
  relocate(Speciality, Total) # Ensure the Total column appears after Speciality


# Create the gt table
gt_table <- final_table %>%
  gt() %>%
  # Rename the Total column to NCL
  ...
  # Apply red color to individual values above 90, except in NCL and System Total
  tab_style(
...
Presentation <- Presentation %>%
  add_slide(layout = "Image and Column Style 2", master = "Content Slide White")  %>%
  ph_with(value = "Key Points", location = ph_location_type(type = "title")) %>% 
  ph_with(value = sentences_list[[1]], location = ph_location_type(type = "Text Placeholder 3"))
