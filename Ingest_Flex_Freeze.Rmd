...
upload_combined_data <- function(con, combined_data) {
  ...
  # Iterate through each provider and date in the freeze data
  for (i in 1:nrow(freeze_data)) {
    provider <- freeze_data$provider_name[i]
    report_date <- freeze_data$ReportDate[i]
    
    # Delete matching flex data from the database
    dbExecute(con, 
              paste("DELETE FROM table_name WHERE DataType = 'flex' AND provider_name = '", provider, 
                    "' AND ReportDate = '", report_date, "'", sep = ""))
  }
  
  # Upload the freeze data to the database
  dbWriteTable(con, "table_name", freeze_data, append = TRUE, row.names = FALSE)
  
  # Upload the new flex data to the database
  dbWriteTable(con, "table_name", flex_data, append = TRUE, row.names = FALSE)
}
