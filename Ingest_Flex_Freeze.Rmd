...
con <- dbConnect(odbc::odbc(), dsn = "SANDPIT", TrustedConnection = TRUE)

# Function to upload combined data
upload_combined_data <- function(con, combined_data) {
  ...
  # Iterate through each row of freeze_data
  for (i in 1:nrow(freeze_data)) {
    provider <- freeze_data$provider_name[i]
    report_date <- freeze_data$ReportDate[i]
    
    # Check if there is matching flex data in the database for the current provider and report_date
    existing_flex <- dbGetQuery(con, 
                                "SELECT COUNT(*) FROM [Data_Lab_NCL].[MohanA].[DashboardTest] 
                                 WHERE DataType = 'flex' AND provider_name = ? AND ReportDate = ?",
                                params = list(provider, report_date))
    
    # If there is matching flex data, delete it
    if (existing_flex > 0) {
      delete_statement <- "DELETE FROM [Data_Lab_NCL].[MohanA].[DashboardTest] 
                           WHERE DataType = 'flex' AND provider_name = ? AND ReportDate = ?"
      dbExecute(con, delete_statement, params = list(provider, report_date))
    }
    
    # Upload the current row of freeze_data to the database
    dbWriteTable(con, "[Data_Lab_NCL].[MohanA].[DashboardTest]", 
                 freeze_data[i, ], append = TRUE, row.names = FALSE)
  }
  
  # Upload all flex data to the database
  dbWriteTable(con, "[Data_Lab_NCL].[MohanA].[DashboardTest]", 
               flex_data, append = TRUE, row.names = FALSE)
}
