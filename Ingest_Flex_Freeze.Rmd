...
## Creating Intermediate Excel File for QA
```{r}
...
tryCatch({
  ...
  if (nrow(wide_freeze_data) > 0) {
    ...
    # Query to check for existing freeze data
    freeze_query <- sprintf(
      "SELECT provider_name, ReportDate FROM [Data_Lab_NCL].[MohanA].[DashboardTestWide] 
       WHERE DataType = 'freeze' AND provider_name IN (%s) AND ReportDate IN (%s)",
      freeze_provider_names, freeze_report_dates
    )
  ...
  if (nrow(wide_flex_data) > 0) {
    delete_statement <- sprintf(
      "DELETE FROM [Data_Lab_NCL].[MohanA].[DashboardTestWide] 
       WHERE DataType = 'flex' 
       AND provider_name IN (%s)
       AND ReportDate IN (%s)",
      flex_provider_names, flex_report_dates
  ...
  if (nrow(wide_freeze_data) > 0) {
    dbAppendTable(con, Id(schema = "MohanA", table = "DashboardTestWide"), wide_freeze_data)
  }

  # Insert wide-format flex data
  if (nrow(wide_flex_data) > 0) {
    dbAppendTable(con, Id(schema = "MohanA", table = "DashboardTestWide"), wide_flex_data)
  }
