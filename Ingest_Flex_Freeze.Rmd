...
wide_data <- wide_data %>%
  mutate(unscheduled = as.numeric(unscheduled))

wide_data <- wide_data %>%
  mutate(total_dna = as.numeric(total_dna))

wide_data <- wide_data %>%
  mutate(unscheduled = ifelse(is.na(unscheduled), "", unscheduled))

wide_data <- wide_data %>%
  mutate(total_dna = ifelse(is.na(total_dna), "", total_dna))
...
tryCatch({
  ...
  if (nrow(wide_freeze_data) > 0) {
    ...
    freeze_query <- sprintf(
      "SELECT provider_name, reportdate FROM [Data_Lab_NCL].[dbo].[diagnostics_combined_data] 
  ...
  if (nrow(wide_flex_data) > 0) {
    delete_statement <- sprintf(
      "DELETE FROM [Data_Lab_NCL].[dbo].[diagnostics_combined_data] 
       WHERE datatype = 'flex' 
  ...
  if (nrow(wide_freeze_data) > 0) {
    dbAppendTable(con, Id(schema = "dbo", table = "diagnostics_combined_data"), wide_freeze_data)
  }

  # Insert wide-format flex data
  if (nrow(wide_flex_data) > 0) {
    dbAppendTable(con, Id(schema = "dbo", table = "diagnostics_combined_data"), wide_flex_data)
  }
